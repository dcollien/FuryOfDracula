...
var isLegalDraculaMove = function( state, move ) {
   ...
   if ( !move ) {
	  ...
	  var oldestTrailIndex = pastMoves.length - constants.trailFullSize;
	  
	  if ( oldestTrailIndex < 0 ) {
		 oldestTrailIndex = 0;
	  }
	  
	  var trail = gameTrail.getTrailMoves( state );
	  
	  if ( move === constants.hideMove ) {
		 if ( list.contains( trail, constants.hideMove ) ) {
			// Dracula can make a HIDE move if he has not already made one in his trail,
			isLegal = false;
			reasonDebug = 'Already made a hide';
		 } else if ( gameMap.locationInfo[dracula.location].isSea ) {
			// he cannot HIDE at sea.
			isLegal = false;
			reasonDebug = 'Cannot hide at sea';
		 }
